...
# Identifiant poli pour ne pas se faire bloquer par les serveurs
HEADERS_POLIS = {'User-Agent': 'RadarUrbexApp/1.0 (Exploration Data)'}
...
if st.button("🚀 Lancer le Scan", use_container_width=True):
    ...
    geolocator = Nominatim(user_agent="radar_urbex_pro_app")
    ...
    if not location:
        ...
        st.success(f"📍 {ville} localisée. Scan sécurisé en cours (veuillez patienter)...")
        
        # 2. Recherches avec PAUSES ANTI-BLOCAGE
        with st.spinner('Fouille des bases de données (Mode Anti-Blocage activé)...'):
            l_osm_indus = chercher_osm_industrie(lat_centre, lon_centre, rayon_m)
            time.sleep(1.5) # On attend 1.5 seconde pour ne pas fâcher le serveur
        ...
        st.info(f"✅ Terminé ! {total_trouvailles} lieux trouvés.")
        ...
        # 4. Affichage (avec le blocage du rechargement)
