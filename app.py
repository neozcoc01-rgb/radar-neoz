...
st.set_page_config(page_title="Radar Urbex Max", page_icon="☢️")
st.title("☢️ Radar Urbex : Édition MAX")
st.markdown("Recherche totale : Industrie, Résidentiel, Militaire, Hôpitaux, Églises, Loisirs & Archives.")

HEADERS_POLIS = {'User-Agent': 'RadarUrbexApp/MaxEdition'}

# --- 1. OSM : INDUSTRIE & FRICHES ---
def chercher_osm_industrie(lat, lon, rayon_m):
    ...
    try: return requests.get(url, params={'data': query}, headers=HEADERS_POLIS, timeout=15).json().get('elements', [])
    except: return []

# --- 2. OSM : MILITAIRE & TRANSPORTS ---
def chercher_osm_militaire(lat, lon, rayon_m):
    ...
    query = f"""[out:json];(
      ...
      nwr["abandoned:railway"](around:{rayon_m},{lat},{lon});
      nwr["building"="train_station"]["disused"="yes"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:building"="train_station"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:amenity"="fuel"](around:{rayon_m},{lat},{lon});
    );out center;"""
    try: return requests.get(url, params={'data': query}, headers=HEADERS_POLIS, timeout=15).json().get('elements', [])
    except: return []

# --- 3. OSM : MÉDICAL, RELIGIEUX & LOISIRS (Nouveau !) ---
def chercher_osm_urbex_special(lat, lon, rayon_m):
    url = "http://overpass-api.de/api/interpreter"
    query = f"""[out:json];(
      nwr["abandoned:amenity"="hospital"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:amenity"="clinic"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:amenity"="place_of_worship"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:leisure"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:tourism"="hotel"](around:{rayon_m},{lat},{lon});
      nwr["abandoned:amenity"="cinema"](around:{rayon_m},{lat},{lon});
    );out center;"""
    try: return requests.get(url, params={'data': query}, headers=HEADERS_POLIS, timeout=15).json().get('elements', [])
    except: return []

# --- 4. ARCHIVES : MÉRIMÉE (Gouvernement Français) ---
def chercher_base_merimee(lat, lon, rayon_m):
    url = "https://data.culture.gouv.fr/api/records/1.0/search/"
    params = {"dataset": "merimee-immeubles-proteges-au-titre-des-monuments-historiques", "q": "ruine OR désaffecté OR friche OR vestige OR abandonné", "geofilter.distance": f"{lat},{lon},{rayon_m}", "rows": 60}
...
def chercher_wikidata(lat, lon, rayon_km):
    ...
    # Q273151: Abandoned building, Q109607: Ruins, Q17028135: Ghost town, Q17351664: Abandoned railway station
    query = f"""SELECT ?place ?placeLabel ?location WHERE {{ SERVICE wikibase:around {{ ?place wdt:P625 ?location . bd:serviceParam wikibase:center "Point({lon} {lat})"^^geo:wktLiteral . bd:serviceParam wikibase:radius "{rayon_km}" . }} ?place wdt:P31/wdt:P279* ?type . VALUES ?type {{ wd:Q273151 wd:Q109607 wd:Q17028135 wd:Q17351664 }} SERVICE wikibase:label {{ bd:serviceParam wikibase:language "fr,en". }} }}"""
    try: return requests.get(url, params={'format': 'json', 'query': query}, headers=HEADERS_POLIS, timeout=15).json()['results']['bindings']
    except: return []

# --- 6. ARCHIVES : WIKIPÉDIA ---
def chercher_wikipedia(lat, lon, rayon_m):
    ...
    mots_cles = ["ruine", "abandonné", "désaffecté", "friche", "vestige", "bunker", "fort", "asile", "sanatorium"]
...
# --- 7. IMAGES : WIKIMEDIA COMMONS ---
def chercher_wikimedia_commons(lat, lon, rayon_m):
    url = "https://commons.wikimedia.org/w/api.php"
    params = {"action": "query", "list": "geosearch", "gscoord": f"{lat}|{lon}", "gsradius": min(rayon_m, 10000), "gslimit": 80, "format": "json"}
    mots_cles = ["abandoned", "ruin", "friche", "decay", "derelict", "urbex", "bunker", "asylum", "sanatorium"]
...
# --- MOTEUR DE RECHERCHE ---
if st.button("☢️ Lancer le Scan Ultime", use_container_width=True):
    geolocator = Nominatim(user_agent="radar_urbex_max")
    ...
    if not location:
        ...
        st.success(f"📍 {ville} localisée. Ratissage maximum en cours (peut prendre 30 secondes)...")
        
        with st.spinner('Interrogation de toutes les bases de données...'):
            ...
            time.sleep(1)
            l_osm_special = chercher_osm_urbex_special(lat_centre, lon_centre, rayon_m)
            time.sleep(1)
            ...
            total = len(l_osm_indus) + len(l_osm_mili) + len(l_osm_special) + len(l_merimee) + len(l_wiki_data) + len(l_wikipedia) + len(l_photos)
            
        st.info(f"✅ Terminé ! {total} lieux potentiels trouvés.")

        # --- CRÉATION DE LA CARTE ---
        carte = folium.Map(location=[lat_centre, lon_centre], zoom_start=12)

        g_indus = folium.FeatureGroup(name='Industrie, Maisons & Friches - Rouge')
        g_mili = folium.FeatureGroup(name='Bunkers, Forts & Transports - Vert')
        g_special = folium.FeatureGroup(name='Hôpitaux, Églises & Loisirs - Violet')
        g_archives = folium.FeatureGroup(name='Archives (État & Wiki) - Noir')
        ...
        def ajouter_points(liste, groupe, couleur, source_nom, icone='info-sign'):
            for l in liste:
                ...
                # Format OSM
                    ...
                    if tags.get('abandoned:amenity') == 'hospital': type_devine = "Hôpital Abandonné"
                    elif tags.get('abandoned:amenity') == 'place_of_worship': type_devine = "Édifice Religieux Abandonné"
                    elif tags.get('abandoned:tourism') == 'hotel': type_devine = "Hôtel Abandonné"
                    elif tags.get('abandoned:leisure'): type_devine = "Lieu de Loisir Abandonné"
                    elif tags.get('landuse') == 'brownfield': type_devine = "Friche Industrielle"
        ...
        ajouter_points(l_osm_indus, g_indus, 'red', 'OpenStreetMap', 'home')
        ajouter_points(l_osm_mili, g_mili, 'darkgreen', 'OpenStreetMap', 'flag')
        ajouter_points(l_osm_special, g_special, 'purple', 'OpenStreetMap', 'star')
        ...
        for g in [g_indus, g_mili, g_special, g_archives, g_photos]:
            g.add_to(carte)
